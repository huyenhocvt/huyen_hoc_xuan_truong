...
def lap_cong_viec():
    if request.method == "POST":
        ...
        append_to_sheet("cong_viec", "Sheet1!A2", row)
        return redirect("/lap-cong-viec")

    ds_nguoi = [row[0] for row in read_sheet("danh_sach", "Sheet1!A2:A") if row]
    ds_loai = sorted(set([row[4] for row in read_sheet("cong_viec", "Sheet1!E2:E") if len(row) >= 5]))
...
def them_nguoi_moi():
    if request.method == "POST":
        ...
        append_to_sheet("danh_sach", "Sheet1!A2", [[ten]])
        return redirect("/lap-cong-viec")
...
def cong_viec_gap():
    ...
    data = read_sheet("cong_viec", "Sheet1!A2:F")
    kq = []
    for row in data:
        if len(row) >= 3:
            try:
